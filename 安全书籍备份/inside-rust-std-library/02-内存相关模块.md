# RUST标准库内存相关模块代码分析
理解RUST程序的最关键点就是理解RUST内存相关的标准库代码。内存标准库代码很好的阐明了RUST的最基本的一些规则如所有权转移，借用，生命周期等。
...
```
`PhantomData`的含义英文如下：
 Zero-sized type used to mark things that "act like" they own a `T`。后文还有分析。
...
```
可见，assume_init_read 行为实际上是从一个已有类型生成并复制一个新的变量。此时原有的MaybeUninit变量可丢弃。 
...
```rust
    pub const fn write(&mut self, val: T) -> &mut T {
        ...
        //但因为MaybeUninit<T>封装的变量不会被drop。所以下面这个等式实际上隐含了 *self必须是未初始化的，否则的话，
        //这里会丢失掉已初始化的变量所有权信息，可能造成内存泄漏。
...
```rust
    pub fn try_new_uninit_in(alloc: A) -> Result<Box<mem::MaybeUninit<T>, A>, AllocError> {
        ...
        //?返回NonNull<[u8]>, NonNull<[u8]>::<MaybeUninit<T>>::cast()返回NonNull<MaybeUninit<T>>
...
这个函数中，如果dst已经初始化过，那原dst变量的所有权将被丢失掉，如果T类型中有引用或指针类型，会引发内存泄漏。 
```rust
pub const fn 
